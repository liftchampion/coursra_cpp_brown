...
vector<Spending> LoadFromJson(istream& input) {
  // Реализуйте эту функцию с помощью библиотеки json.h
	...
	result.reserve(doc.GetRoot().AsArray().size());
	for (const auto& entry : doc.GetRoot().AsArray()) {

		result.push_back({entry.AsMap().at("category").AsString(),
						  entry.AsMap().at("amount").AsInt()});
	}
	return result;
}
